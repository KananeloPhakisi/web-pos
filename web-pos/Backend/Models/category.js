...
exports.getCategories = (req, page) => {
  const sql = "SELECT * FROM table_categories";
  return new Promise((resolve, reject) => {
    getMaxPage(page, null, "table_categories")
      .then(maxPage => {
        const infoPage = {
...
exports.getCategoryByName = req => {
  return new Promise((resolve, reject) => {
    ...
    conn.query(
      "SELECT * FROM table_categories WHERE name = ?",
      [categoryName],
      (err, result) => {
          if(!err) resolve(result);
          else reject(err);
...
exports.newCategory = req => {
  return new Promise((resolve, reject) => {
    conn.query(
      `INSERT INTO table_categories SET name = ?`,
...
exports.updateCategory = req => {
  return new Promise((resolve, reject) => {
    conn.query(
      `UPDATE table_categories SET name = ? WHERE id = ?`,
...
exports.updateUncategorized = categoryId => {
    return new Promise((resolve, reject) => {
        conn.query(`UPDATE table_products SET category = 1 WHERE category = ?`, [categoryId], (err, result) => {
            if(!err) resolve(result);
            else reject(result);
        })
    })
