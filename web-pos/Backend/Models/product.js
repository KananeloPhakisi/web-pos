...
exports.getProducts = (req, page) => {
  ...
  return new Promise((resolve, reject) => {
    getMaxPage(page, query.keyword, "table_products")
      .then(maxPage => {
        const infoPage = {
  ...
  return new Promise((resolve, reject) => {
    ...
    conn.query(
      `SELECT product.id, product.name as product_name, product.description, product.image,
        category.name as category, product.price, product.quantity, product.created_at, product.updated_at FROM table_products as product, 
        table_categories as category WHERE product.category = category.id AND product.name = ?`,
      [prodName],
      (err, result) => {
        if (!err) resolve(result);
        else reject(err);
...
exports.getProductByCategoryId = req => {
    return new Promise((resolve, reject) => {
        ...
        category.name as category, product.price, product.quantity, product.created_at, product.updated_at FROM table_products as product, 
        table_categories as category WHERE product.category = category.id AND category.id = ?`), [req.body.category_id],
        (err, result) => {
            if(!err) resolve(result);
            else reject(err);
        }
...
exports.newProduct = req => {
  ...
  return new Promise((resolve, reject) => {
    conn.query(
      `INSERT INTO table_products SET name = ?, description = ?, image = ?, category = ?, price = ?, quantity = ?`,
      [
        body.prod_name,
        body.prod_desc,
        body.prod_image,
...
exports.updateProduct = req => {
  ...
  return new Promise((resolve, reject) => {
    conn.query(
      `UPDATE table_products SET name = ?, description = ?, image = ?, category = ?, price = ?, quantity = ? WHERE id = ?`,
      [
        body.prod_name,
        body.prod_desc,
        body.prod_image,
...
exports.deleteProduct = req => {
  return new Promise((resolve, reject) => {
    ...
    conn.query(
      `DELETE FROM table_products WHERE id = ?`,
      [prodId],
      (err, result) => {
        if (!err) resolve(result);
        else reject(err);
