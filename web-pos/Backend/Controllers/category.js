...
exports.newCategory = (req, res) => {
  if (req.body.category_name == null || req.body.category_name == "")
    return response.error(res, "Category name cannot be empty!");

  model.getCategoryByName(req).then(resultName => {
    if (resultName.length !== 0)
      return response.error(res, "Category already exists!");
    model
      .newCategory(req)
      .then(resultNew => {
...
exports.updateCategory = (req, res) => {
  ...
  if (req.params.category_id == "1")
    return response.error(
      res,
      "You cannot edit an uncategorized category!"
    );
    ...
    .then(resultId => {
      ...
      model.getCategoryByName(req).then(resultName => {
        if (
          ...
          return response.error(res, "Category name already exists!");
        model
          .updateCategory(req)
          .then(resultUpdate => {
            model
